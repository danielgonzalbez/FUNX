...
It supports the most common arithmetic operands (+, -, *, /, %, ^) as well as some logical operators (=, !=, <, >, <=, >=). It also supports the boolean operators 'and' and 'or'. They all return 1 if an expression is true and 0 in the opposite case. The boolean expressions 'true' and 'false' are also supported.
...
The inputs and outputs of the Funx interpreter are processed via a web page. This service has been developed with ```flask``` in combination with the library ```Jinja2``` to generate and render HTML templates (whose style is in the file main.css in the static folder). 
